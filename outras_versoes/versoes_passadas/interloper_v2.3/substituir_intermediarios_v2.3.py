...
class FrameInterpolationModel(nn.Module):
    def __init__(self):
        ...
        self.encoder = nn.Sequential(
            nn.Conv2d(6, 128, kernel_size=14, padding=1, stride=2),  # Reduz a resolução pela metade
            nn.PReLU(),
            nn.Conv2d(128, 256, kernel_size=7, padding=1, stride=2),  # Reduz novamente pela metade
            nn.PReLU(),
        ...
        self.decoder = nn.Sequential(
            ...
            nn.PReLU(),
            nn.ConvTranspose2d(256, 128, kernel_size=7, padding=1, stride=2, output_padding=1),  # Aumenta novamente
            nn.PReLU(),
            nn.ConvTranspose2d(128, 64, kernel_size=14, padding=1, stride=2, output_padding=1),  # Aumenta mais uma vez
            nn.PReLU(),
    ...
    for i in range(0, len(frames) - 2, 2):
        ...
        with torch.no_grad():
            with autocast(device_type=str(device)):
                ...
                # Adicione o redimensionamento para corresponder ao tamanho do frame original
                output = F.interpolate(output, size=frame1.shape[2:], mode='bilinear', align_corners=True)
                frame2_interpolated = output.squeeze(0).permute(1, 2, 0).cpu().numpy()
...
def main(video_path, output_path, model_path="C:\\Users\\Felip\\Desktop\\Interloper\\versoes_passadas\\interloper_v2.3\\frame_interpolation_model_v2_3.pth"):
...
if __name__ == "__main__":
    video_path = 'C:\\Users\\Felip\\Desktop\\Interloper\\videos_30fps\\eldenring.mp4'
    output_path = 'C:\\Users\\Felip\\Desktop\\Interloper\\testes_videos\\eldenring_v2_3_GEN5.mp4'
