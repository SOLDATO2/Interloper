...
def resize_image(image, max_size=600):
    """
    Redimensiona a imagem para que a maior dimensão seja max_size pixels.
    Mantém a proporção.
    """
    h, w, c = image.shape
    if max(h, w) > max_size:
...
class FrameInterpolationModel(nn.Module):
    def __init__(self):
        ...
        self.encoder = nn.Sequential(
            nn.Conv2d(8, 128, kernel_size=7, padding=3, stride=2),  # 6 para frames + 2 para fluxo
            nn.PReLU(),
        ...
        self.decoder = nn.Sequential(
            ...
            nn.PReLU(),
            nn.ConvTranspose2d(128, 64, kernel_size=7, padding=3, stride=2, output_padding=1),
            nn.PReLU(),
            nn.Conv2d(64, 3, kernel_size=3, padding=1, stride=1),
        )

    def forward(self, frame1, frame3, flow):
        # Concatenar os frames com o fluxo óptico
        x = torch.cat([frame1, frame3, flow], dim=1)  # [B, 8, H, W]
        x = self.encoder(x)
        x = self.decoder(x)
        return x
            ...
            for batch_idx, (frame1, frame3, flow, frame2) in enumerate(dataloader):
                ...
                if (batch_idx + 1) % display_step == 0:
                    try:
                        ...
                        frame1_img = resize_image(frame1_img)
                        frame3_img = resize_image(frame3_img)
                        output_img = resize_image(output_img)
                        frame2_img = resize_image(frame2_img)
                        flow_img = resize_image(flow_img)
