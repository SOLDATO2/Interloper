...
import math
import numpy as np
# importe correlation.py se estiver no mesmo dir
from .correlation.correlation import FunctionCorrelation

# MANTÉM: todo o código existente (funções, kernels, etc.)
# EXCETO a parte do if __name__ == '__main__' pois vamos transformar em classe.
...
try:
    ...
    sys.path.insert(0, './correlation'); import LiteFlowNet.correlation.correlation as correlation # you should consider upgrading python
# end
...
class Network(torch.nn.Module):
    def __init__(self):
        ...
        self.load_state_dict({ strKey.replace('module', 'net'): tenWeight for strKey, tenWeight in torch.hub.load_state_dict_from_url(url='http://content.sniklaus.com/github/pytorch-liteflownet/network-' + args_strModel + '.pytorch', file_name='liteflownet-' + args_strModel).items() })
    # end
...
class LiteFlowNet(torch.nn.Module):
    def __init__(self, model='default'):
        super().__init__()
        self.args_strModel = model
        self.netNetwork = Network()  # a classe Network definida no run.py
        self.netNetwork = self.netNetwork.cuda().eval()  # ou .to(device)

    def forward(self, frame1, frame2):
        """
        Recebe frame1, frame2 no formato [B,3,H,W] (float32, 0..1 ou algo similar).
        Retorna fluxo [B,2,H,W].
        """
        # Dimensões originais
        b, c, h, w = frame1.shape
        
        # Se quiser remover as checagens de "assert" que existiam, pode comentar:
        # (No original, havia a restrição de 1024x436.)
        
        # Ajuste para a "interpolação" que a rede faz:
        intPreprocessedWidth = int(math.floor(math.ceil(w / 32.0) * 32.0))
        intPreprocessedHeight = int(math.floor(math.ceil(h / 32.0) * 32.0))
        
        # Resize para múltiplos de 32
        frame1_up = torch.nn.functional.interpolate(
            input=frame1, 
            size=(intPreprocessedHeight, intPreprocessedWidth),
            mode='bilinear', align_corners=False
        )
        frame2_up = torch.nn.functional.interpolate(
            input=frame2,
            size=(intPreprocessedHeight, intPreprocessedWidth),
            mode='bilinear', align_corners=False
        )

        # Usa o netNetwork do LiteFlowNet
        flow_up = self.netNetwork(frame1_up, frame2_up)  # [B,2,H_up,W_up]

        # Redimensiona de volta para [H,W] original
        ...
        # Ajusta escala do fluxo
        flow[:, 0, :, :] *= float(w) / float(intPreprocessedWidth)
        flow[:, 1, :, :] *= float(h) / float(intPreprocessedHeight)
